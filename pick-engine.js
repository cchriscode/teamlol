/**
 * pick-engine.js — Pick recommendation algorithm (browser prototype).
 *
 * Implements the v3 algorithm from docs/pick-recommend-spec.md §5.
 * Inputs: DraftState. Outputs: RecommendResult.
 *
 * Pure function: given the same DraftState + PickData, returns the same result.
 * No network calls, no DOM access. UI lives in pick-recommend.html.
 */
(function () {
  'use strict';

  if (!window.PickData) {
    console.error('[pick-engine] PickData missing — load pick-data.js first');
    return;
  }
  const D = window.PickData;
  ...
  window.PickEngine = {
    ...
    archetypeKr,
    laneKr,
    botArchetypeKr: (a) => (D.BOT_DUO_ARCHETYPES && D.BOT_DUO_ARCHETYPES[a] && D.BOT_DUO_ARCHETYPES[a].label) || a,
  };
})();
