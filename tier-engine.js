/**
 * tier-engine.js — Champion tier scoring (PS Score + helpers).
 *
...
(function () {
  ...
  if (!window.PickData) {
    console.error('[tier-engine] PickData missing — load pick-data.js first');
    return;
  }
  ...
  window.TierEngine = {
    ...
    tierTable,
    fullTable,
    // Constants (read-only via accessor)
    WEIGHTS,
    SAMPLE_THRESHOLDS,
    LETTER_BANDS,
  };
